...
func _load_main_menu() -> void:
	...
	for menu_item: String in config.get_section_keys("main_menu"):
		...
		new_menu_button.text = TFT.get_text("menu." + menu_name.to_snake_case(), "res://data/translation.csv")
		...
		for item: Dictionary in main_menu_data[menu_item]:
			...
			match item.get("type", 0):
				0: # original option
					new_menu_button.get_popup().add_item(TFT.get_text(item.get("key", ""), "res://data/translation.csv"), item.get("code", -1))
				1: # submenu
						...
						_: # just load items to another popup menu for other submenus
							for submenu_item: Dictionary in config.get_value("main_menu", item.get("text", "").to_snake_case() + "_submenu"):
								...
								match submenu_item.get("type", 0):
									0: # original option
										submenu.add_item(TFT.get_text(submenu_item.get("key", ""), "res://data/translation.csv"), submenu_item.get("code", -1))
									-1: # seperator
										submenu.add_separator(TFT.get_text(submenu_item.get("key", ""), "res://data/translation.csv"))
					...
					# add submenu
					new_menu_button.get_popup().add_submenu_node_item(TFT.get_text(item.get("key", ""), "res://data/translation.csv"), submenu, item.get("code", -1))
					# disable empty submenus
					if submenu.item_count == 0:
						new_menu_button.get_popup().set_item_disabled(-1, true)
				-1: # seperator
					new_menu_button.get_popup().add_separator(TFT.get_text(item.get("key", ""), "res://data/translation.csv"))
				2: # checkable
					new_menu_button.get_popup().add_check_item(TFT.get_text(item.get("key", ""), "res://data/translation.csv"), item.get("code", -1))
				3: # radio
					new_menu_button.get_popup().add_radio_check_item(TFT.get_text(item.get("key", ""), "res://data/translation.csv"), item.get("code", -1))
