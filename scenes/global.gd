...
func get_editor_text() -> String:
	return get_main_node().editor.text

func set_editor_text(text: String) -> void:
	get_main_node().editor.text = text

func set_editor_disabled(disabled: bool) -> void:
	get_main_node().editor.editable = not disabled

func is_editor_disabled() -> bool:
	return not get_main_node().editor.editable

func get_main_node() -> App_MainWindow:
